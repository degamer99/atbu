  ...
  const uploadFile = (e) => {
    ...
    for (let i = 0; i < posts.length; i++) {
      if (name != ""){
        ...
        let isThere = posts[i].name.search(name.toUpperCase())
  ...
  const search = async (e) =>{
    ...
    for (let i = 0; i < posts.length; i++) {
      if (e.target.value != ""){
        ...
        let isThere = posts[i].name.search(e.target.value.toUpperCase())
        
        if (isThere != -1 ){
          x.push({name: posts[i].name, url: posts[i].url})
          setResults(x)
          console.log(posts[i])
          console.log(results)
        }
      ...
      <main>
          ...
          <div className={styles.search}>
            <div>
              <svg focusable="false" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M15.5 14h-.79l-.28-.27A6.471 6.471 0 0 0 16 9.5 6.5 6.5 0 1 0 9.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z"></path></svg>
                <input type="text"  name='search' list="autoResults" className={styles.input} placeholder='Search . . .' onChange={ search }/>
        ...
        <ul className={styles.results}>
          { results.map( (value, index, array ) => {
            ...
            return (
              <li key={index} value={value} onClick={ () => window.location.assign(value.url)}>
                  <svg focusable="false" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M15.5 14h-.79l-.28-.27A6.471 6.471 0 0 0 16 9.5 6.5 6.5 0 1 0 9.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z"></path></svg>
                   <p>{value.name}</p> 
                </li>
            )} )
          ...
          {/* <button value="Download" className={ !isUrl ? styles.buttonA : styles.buttonB} onClick={ () => downloadRef.current.click()}> Download </button> */}
          <button className={uploadState? styles.buttonLoad : styles.buttonB} onClick={uploadChange}> {uploadState? "Uploading..." : "Upload"} </button>
        </section>
        { href ?  <a href={href}> click to download </a> : <a> </a>}
        <input type="file" name='upload' ref={fileRef} style={{display : "none"}} onChange={uploadFile} />
      </main>
    </>
  )
}
